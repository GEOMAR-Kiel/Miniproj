...
pub fn get_ellipsoids(db: &MemoryDb) -> Result<HashMap<u32, Ellipsoid>, Box<dyn Error>> {
    ...
    for a in &ell_rows {
        ...
        let ellipsoid = match a {
            ...
            _ => {
                return Err(format!(
                    "Malformed DB: Ellipsoids need either b or f_inv. (row: {a:?}"
                )
                .into());
            }
        };
...
pub fn gen_parameter_constructors(
    ...
    let units = db
        ...
        .filter_map(|row| match row {
            [
                Some(Field::IntLike(uom_code)),
                Some(Field::Double(factor_b)),
                Some(Field::Double(factor_c)),
        ...
        .filter_map(|row| {
            match row {
                ...
                [
                    ...
                    Some(Field::IntLike(conv_code)),
                    _,
                    Some(Field::StringLike("projected")),
                ] => Some((
                    u32::try_from(code).ok()?,
                    CrsEntry::Projected {
                        conversion: u32::try_from(conv_code).ok()?,
                        base: u32::try_from(base_crs_code).ok()?,
                    },
                )),
                [
                    Some(Field::IntLike(code)),
    ...
    let extents_table = db
        ...
        .filter_map(|row| {
            match row {
                [
                    Some(Field::IntLike(code)),
                    Some(Field::StringLike(name)),
                    Some(Field::Double(lat_s)),
                    Some(Field::Double(lon_w)),
                    Some(Field::Double(lat_n)),
                    Some(Field::Double(lon_e)),
                ] => {
                    Some((
                        u32::try_from(code).ok()?,
                        (name, [lon_e, lat_n, lon_w, lat_s]),
    ...
    let op_table = db
        ...
        .filter_map(|row| {
            let [
                ...
                Some(Field::IntLike(coord_op_method_code)),
            ] = row
            else {
                return None;
            };
    ...
    db.get_table("epsg_coordoperationparamvalue")
        ...
        .try_for_each::<_, Result<_, Box<dyn Error>>>(|row| {
            match row {
                [
                    Some(Field::IntLike(coord_op_code)),
                    Some(Field::IntLike(parameter_code)),
                    Some(Field::Double(v)),
                    Some(Field::IntLike(9110)),
                ] => {
                    paramvalues
    ...
    let datum_table = db
        ...
        .filter_map(|row| {
            ...
            match (
                u32::try_from(code),
                u32::try_from(ellipsoid_code),
                u32::try_from(prime_meridian_code),
            ) {
                (Ok(code), Ok(ellipsoid_code), Ok(8901)) => {
                    // since correction for other meridians is currently missing.
