        ...
        .mapped(|r| {
            {
                ...
                let greenwich_relative =
                    r.get_unwrap::<_, Option<f64>>("g_conv").unwrap_or_else(|| {
                ...
                phf_map.entry(
                    code,
                    &format!("f64::from_bits(0x{:x})", greenwich_relative.to_bits()),
                );
            };
            Ok(())
        })
...
pub fn gen_parameter_constructors(
    ...
    let crs_table = db.get_table("epsg_coordinatereferencesystem")
        ...
        .map(|row| {
            let [Some(Field::IntLike(code)), base_crs_code, projection_conv_code, Some(Field::IntLike(datum_code)), Some(Field::StringLike(crs_kind))] = row else {return Err(format!("Missing code in {row:?}").into())};
